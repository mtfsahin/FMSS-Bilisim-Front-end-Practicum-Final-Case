...
export default function DetailsComponent({ url }) {
    ...
    useEffect(() => {
        ...
        const fetchStarship = async () => {
            try {
                //get url enviroment file & add starshipID 
                const response = await axios.get(`${process.env.REACT_APP_URL} + ${starshipID}`);
                //response data
    ...
    return (
        <div className="px-5 sm:px-20 ">
            <div className="container mt-10 p-10 bg-white backdrop-blur-sm bg-opacity-20 rounded-md 0">
                {/* check loading */}
                {loading ? (
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap my-8">
                            <div className="w-full p-10  md:w-2/5 md:pr-10 rounded-s-2xl bg-primary-950">
                                {/* Ship image area */}
                                <img
                                    className="w-72 mt-8"
                                    alt="starship_image"
